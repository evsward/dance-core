...
public class StrutsUtils {
	...
	/***
	 * 通过POI直接输出Execl2007以上的xlsx文件
	 * eg . 
	 * reader(xsswb, "encoding:UTF-8")
	 * @param xsswb POI的XSSFWorkbook excel2007文件处理类
	 * @param headers
	 */
	public static void reander(final XSSFWorkbook xsswb, final String... headers) {
		HttpServletResponse response = initResponseHeader(ServletUtils.EXCEL_TYPE, headers);
		try {
			xsswb.write(response.getOutputStream());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
